	...
	<section class="content-header">
		<div class="content-fluid">
			<div class="row">
				<div class="col-12">
					<h1>
						<i class="fa fa-trash-alt"></i> Tempat Sampah <button class="btn btn-danger float-right" data-toggle="modal" data-target="#kosongkan" title="Kosongkan Tempat Sampah">Kosongkan Tempat Sampah</button>
					</h1>
				</div>
			</div>
		</div>
	</section>
    <div class="modal fade" id="kosongkan">
        <div class="modal-dialog">
        <form action="<?php echo base_url(); ?>shiroki_trash_manifest" method="POST">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Kosongkan Tempat Sampah</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body">
                    Apakah anda yakin ingin menghapus seluruh isi tempat sampah ?
                </div>
                <div class="modal-footer justify-content-between">
                    <input type="hidden" name="tid" value="<?php echo $this->encrypt_model->my_encrypt(date('U')); ?>">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    <button type="submit" class="btn btn-danger"><i class="fa fa-check"></i> Kosongkan</button>
                </div>
            </div>
        </form>
        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        table = $('#table').DataTable({ 
            ...
            stateSave: true,
            ajax: {
                url: "<?php echo site_url('shiroki_manifest_bin_data_ajax')?>",
                type: "POST",
                data: function ( data ) {
